...
def write_sheet(df: pd.DataFrame, excel_path: Path, sheet_name: str, index: bool = False) -> None:
    ...
    - If the sheet already exists, removes it and writes the new DataFrame
    """
...
def build_consolidated_df(cfg: WarReportConfig, start_dt: datetime, end_dt: datetime) -> pd.DataFrame:
    ...
    for f in input_dir.glob(cfg.file_glob):
        try:
        ...
        for c in df.columns:
            c_norm = str(c).strip()
            if c_norm.lower().startswith('fecha'):
                colmap[c] = 'Fecha Operación'
            elif c_norm.lower() in ('nombre', 'nombre ', 'nome', 'nomb', 'nombre do fundo'):
                colmap[c] = 'NOMBRE'
            else:
                colmap[c] = c_norm
        ...
        if 'Fecha Operación' not in df.columns:
            print(f"File {f.name}: missing 'Fecha Operación' column. Skipping.")
            continue

        df['Fecha Operación'] = pd.to_datetime(df['Fecha Operación'], format='%Y%m%d', errors='coerce')
...
def outliers_by_fund(df: pd.DataFrame, col: str = '%', k: float = 2.0) -> pd.DataFrame:
    if ('Fundo' not in df.columns) or (col not in df.columns):
        return pd.DataFrame()
    out_all = []
    for fundo, g in df.groupby('Fundo'):
        mean = g[col].mean()
        std  = g[col].std()
...
def compute_stats_and_rankings(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        agg: Dict[str, List] = {}
        if 'Patrimonio' in df.columns: agg['Patrimonio'] = ['mean','std','min','max']
        if 'COM' in df.columns:        agg['COM']        = ['sum','mean','std']
        if 'VEN' in df.columns:        agg['VEN']        = ['sum','mean','std']
        if 'COM > VEN' in df.columns:  agg['COM > VEN']  = ['mean']
        if 'COM < VEN' in df.columns:  agg['COM < VEN']  = ['mean']
        if '%' in df.columns:          agg['%']          = ['mean','std','min','max', std_positive, std_negative]
    else:
        agg = {
    ...
    return stats, r.sort_values(by=r.columns[0] if len(r.columns) else None) if not r.empty else r
...
def correlation_tables(df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    ...
    """
    if not {'Fundo','Data','Patrimonio'}.issubset(df.columns):
        return {
    ...
    # top table with labels
    top = pairs.copy()
    top['Rank'] = range(1, len(top) + 1)
    def _fmt(r):
        try:
            return f"Top {r['Rank']}: {r['Fund 1']} × {r['Fund 2']} (corr={r['Correlation']:.2f})"
        except Exception:
            return f"Top {r['Rank']}: {r['Fund 1']} × {r['Fund 2']} (corr=NA)"
...
def export_fund_evolution(df: pd.DataFrame, fund: str, excel_path: Path) -> None:
    ...
    ser = (df[df['Fundo'] == fund].copy().sort_values('Data'))[cols]
    if ser.empty:
        print(f"[Evolution] No rows for '{fund}'.")
        return
    sheet = f"Evolution_{fund}"[:31]
    write_sheet(ser, excel_path, sheet, index=False)
    print(f"[Evolution] Sheet '{sheet}' written ({len(ser)} rows).")

def export_fund_arima(df: pd.DataFrame, fund: str, horizon: int, excel_path: Path) -> None:
    try:
        from statsmodels.tsa.statespace.sarimax import SARIMAX
    except Exception as e:
        print(f"[ARIMA] statsmodels not available: {e}")
        return

    ser = (df[df['Fundo'] == fund][['Data','Patrimonio']]
           .dropna()
           .sort_values('Data')
           .copy())
    if len(ser) < 10:
        print(f"[ARIMA] Too few points for '{fund}' ({len(ser)}). Skipping.")
        return
    ...
    try:
        ...
        print(f"[ARIMA] Sheet '{sheet}' written ({len(out)} rows).")
    except Exception as e:
        print(f"[ARIMA] Error for '{fund}': {e}")
...
def run_war_report(
    ...
    print(f"Analysis period: {start_dt:%Y-%m-%d} to {end_dt:%Y-%m-%d}")
    ...
    if not out_fund.empty: write_sheet(out_fund, excel_path, 'Outliers_Fundo', index=False)
    if not out_day.empty:  write_sheet(out_day,  excel_path, 'Outliers_Dia',   index=False)

    # Stats & Rankings
    stats, ranks = compute_stats_and_rankings(dados)
    if not stats.empty: write_sheet(stats, excel_path, 'Estatisticas', index=True)
    if not ranks.empty: write_sheet(ranks, excel_path, 'Rankings', index=True)
    ...
    print(f"✅ WAR REPORT complete. Excel written at: {excel_path}")
    return excel_path
...
def ensure_dirs_life(cfg: LifeCfg) -> Tuple[Path, Path]:
    ...
    csv_dir = out_dir / cfg.csv_subdir
...
def load_and_prepare(cfg: LifeCfg) -> pd.DataFrame:
    ...
    df['nr_unidades'] = pd.to_numeric(df['nr_unidades'], errors='coerce')
...
def tag_movements(df: pd.DataFrame, cfg: LifeCfg) -> pd.DataFrame:
    ...
    def classify(row):
        if pd.isna(row['prev_nr_unidades']):
            return 'buy' if row['fecha_contenido'] > first_cutoff else 'check_november'
        if row['nr_unidades'] > row['prev_nr_unidades']:
            return 'increase'
        if row['nr_unidades'] < row['prev_nr_unidades']:
            if (row['nr_unidades'] == 0) or pd.isna(row['next_nr_unidades']):
                return 'sell'
            return 'reduce'
        return 'keep'
...
def clients_per_fund(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    per_month = (df.groupby(['mes_ano','des_produto'])['nuc']
                   .nunique().rename('unique_clients').reset_index())
...
def combinations(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    comb_freq = (multi.groupby('funds_combo')
...
def churn_metrics(df_tagged: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if not sells.empty:
        sells['no_reinvestment'] = sells.apply(no_reinvest, axis=1)
        churn_candidates = sells[sells['no_reinvestment']].copy()
        churn_by_month = (churn_candidates.groupby(churn_candidates['fecha_contenido'].dt.to_period('M'))['nuc']
                          .nunique().rename('clients_churn').reset_index())
    else:
        churn_by_month = pd.DataFrame(columns=['fecha_contenido','clients_churn'])

    active_by_month = (df_tagged.groupby(df_tagged['fecha_contenido'].dt.to_period('M'))['nuc']
                       .nunique().rename('active_clients').reset_index())
...
def acquisition_and_retention(df_tagged: pd.DataFrame):
    ...
    acq_global = (buys.groupby(buys['fecha_contenido'].dt.to_period('M'))['nuc']
                  .nunique().rename('clients_buy').reset_index())
    acq_fund   = (buys.groupby([buys['fecha_contenido'].dt.to_period('M'),'des_produto'])['nuc']
                  .nunique().rename('clients_buy').reset_index().rename(columns={'fecha_contenido':'mes_ano'}))
    top_acq_abs = (buys.groupby('des_produto')['nuc'].nunique()
                   .rename('unique_buyers').reset_index()
    ...
    cont_ret = (df_[~df_['next_month'].isna()].groupby(['mes_ano','des_produto'])['nuc']
                .nunique().rename('clients_t_continue').reset_index())
...
def client_profiles(df_tagged: pd.DataFrame) -> pd.DataFrame:
    ...
    freq = (df_[df_['tag'].isin(['buy','sell','increase','reduce'])]
            .groupby('nuc').size().rename('frequency'))
    ...
    last_move = (df_[df_['tag'].isin(['buy','sell','increase','reduce'])]
                 .groupby('nuc')['mes_ano'].max().rename('last_move_month'))
    ...
    profiles = pd.concat([quantity, freq, recency, inactivity, is_current, months_since_churn], axis=1)\
                 .fillna({'frequency':0,'inactivity_months':np.nan})\
                 .reset_index()
    return profiles
...
def flows_transitions(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    df_ = df.copy()
    df_['mes_ano'] = df_['fecha_contenido'].dt.to_period('M')
    holds = (df_.groupby(['nuc','mes_ano'])['des_produto']
    ...
    matrix = (detail.groupby(['from_fund','to_fund']).size()
...
def run_life_cycle(
    ...
    cfg = LifeCfg(
        ...
        first_month_known=first_month_known
    )
    ...
    print("✅ LIFE CYCLE pipeline done.")
